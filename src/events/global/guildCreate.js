    ...
    async run(guild) {
        ...
        client.cluster.broadcastEval(async (client, context) => {
            ...
            if (results.includes(true)) {
                console.log(`[${new Date().toString().split(' ', 5).join(' ')}][GUILD] New Server Joined. Sending Information`);
            }
            else {
                console.log(`[${new Date().toString().split(' ', 5).join(' ')}][GUILD] Joining a discord server. Unable to send message to join-leave logs.`);
            }
        }).catch(console.error);
    }
};
