...
module.exports = class GuildMemberAdd extends Event {
    ...
    async run(member) {
        ...
        if (!serverData.welcome.status) return;
        ...
        const customMessage = `${serverData.welcome.message || 'Enjoy your stay!'}`;

        context.font = 'bold 50px "LEMON MILK", sans-serif';
        context.fillStyle = 'white';
        context.fillText('Welcome', 220, 245);
        ...
        const channel = member.guild.channels.cache.get(serverData.welcome.channel);
        if (!channel) return;
