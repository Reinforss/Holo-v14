...
module.exports.run = async (client, player, track) => {
	...
	const titles = track.info.title.length > 40 ? track.info.title.substr(0, 40) + '...' : track.info.title;
	...
	const Started = new EmbedBuilder()
		...
		.setColor('Aqua')
		.setFooter({ text: `Loop Mode: ${capital(player.loop)} • Queue Left: ${player.queue.length} • Volume: ${curVolume}%` });
	...
	const filter = (message) => {
		if (message.guild.members.me.voice.channel && message.guild.members.me.voice.channelId === message.member.voice.channelId) {
			if (message.user.id === track.info.requester.id) {
				return true;
			}
			else {
				message.reply({
					content: '`❌` | Only the requester of the track can use these buttons.',
					ephemeral: true,
				});
				return false;
			}
	...
	collector.on('collect', async (message) => {
		if (message.customId === 'loop') {
			...
			if (!player) {
				...
				const embed = new EmbedBuilder().setColor('Red').setDescription('`❌` | Song can\'t be replay');
			...
			if (!player) {
				...
				const embed = new EmbedBuilder().setDescription('`❌` | Queue is: `Empty`').setColor('Red');
			...
			if (!player) {
				...
				const embed = new EmbedBuilder().setDescription('`❌` | Volume can\'t be lower than: `10%`').setColor('Red');
			...
			if (!player) {
				...
				const embed = new EmbedBuilder().setDescription('`❌` | Volume can\'t be higher than: `100%`').setColor('Red');
